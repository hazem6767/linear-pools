...
describeForkTest('UnbuttonAaveLinearPoolFactory', 'mainnet', 16015018, function () {
  ...
  const wAMPL = '0xEDB171C18cE90B633DB442f2A6F72874093b49Ef';
  const ubAaveAMPL = '0xF03387d8d0FF326ab586A58E0ab4121d106147DF';
  ...
  before('run task', async () => {
    task = new Task('20230117-unbutton-aave-rebalanced-linear-pool', TaskMode.TEST, getForkedNetwork(hre));
    await task.run({ force: true });
    factory = await task.deployedInstance('UnbuttonAaveLinearPoolFactory');
  });

  before('load signers', async () => {
    [, owner, other] = await getSigners();

    holder = await impersonate(wAMPL_HOLDER, fp(100));
  });

  before('setup contracts', async () => {
    vault = await new Task('20210418-vault', TaskMode.READ_ONLY, getForkedNetwork(hre)).deployedInstance('Vault');

    mainToken = await task.instanceAt('IERC20', wAMPL);
  ...
  function itRebalancesThePool(expectedState: LinearPoolState) {
    it('rebalance the pool', async () => {
      ...
      const { cash } = await vault.getPoolTokenInfo(poolId, wAMPL);
      const scaledCash = cash.mul(wAMPL_SCALING);
      ...
      const mainInfo = await vault.getPoolTokenInfo(poolId, wAMPL);

      const expectedMainBalance = lowerTarget.add(upperTarget).div(2);
      expect(mainInfo.cash.mul(wAMPL_SCALING)).to.equal(expectedMainBalance);
  ...
  describe('create and check getters', () => {
    it('deploy a linear pool', async () => {
      ...
      pool = await task.instanceAt('UnbuttonAaveLinearPool', event.args.pool);
      ...
      const { assetManager } = await vault.getPoolTokenInfo(poolId, wAMPL); // We could query for either wAMPL or ubAaveAMPL
      rebalancer = await task.instanceAt('UnbuttonAaveLinearPoolRebalancer', assetManager);
    ...
    it('check factory version', async () => {
      const expectedFactoryVersion = {
        ...
        version: 1,
        deployment: '20230117-unbutton-aave-rebalanced-linear-pool',
      };
  ...
  describe('join, and rebalance', () => {
    it('join the pool', async () => {
      ...
      const joinAmount = INITIAL_UPPER_TARGET.mul(2).div(wAMPL_SCALING);
      ...
      const excess = joinAmount.mul(wAMPL_SCALING).sub(INITIAL_UPPER_TARGET);
      const joinCollectedFees = excess.mul(SWAP_FEE_PERCENTAGE).div(FP_ONE);

      const expectedBPT = joinAmount.mul(wAMPL_SCALING).sub(joinCollectedFees);
  ...
  describe('generate excess of main token and rebalance', () => {
    it('deposit main tokens', async () => {
      ...
      const joinAmount = upperTarget.mul(5).div(wAMPL_SCALING);
  ...
  describe('generate lack of main token and rebalance', () => {
    it('withdraw main tokens', async () => {
      ...
      const exitAmount = scaledCash.sub(lowerTarget.div(3)).div(wAMPL_SCALING);
  ...
  describe('join below upper target and rebalance', () => {
    it('deposit main tokens', async () => {
      ...
      const joinAmount = midpoint.div(100).div(wAMPL_SCALING);
  ...
  describe('exit above lower target and rebalance', () => {
    it('withdraw main tokens', async () => {
      ...
      const exitAmount = midpoint.div(100).div(wAMPL_SCALING);
  ...
  //     await setCode(USDC_LENDING_POOL, getArtifact('v2-pool-linear/MockUnbuttonAaveLendingPool').deployedBytecode);
  //     const mockLendingPool = await deployedAt('v2-pool-linear/MockUnbuttonAaveLendingPool', USDC_LENDING_POOL);
