...
describeForkTest('UnbuttonAaveLinearPoolFactory', 'mainnet', 15629530, function () {
  ...
  describe('rebalancer query protection', async () => {
    it('reverts with a malicious lending pool', async () => {
      ...
      await setCode(ubAaveAMPL, getExternalPackageArtifact('linear-pools/MockUnbuttonERC20').deployedBytecode);
      const mockLendingPool = await getExternalPackageDeployedAt('linear-pools/MockUnbuttonERC20', ubAaveAMPL);
