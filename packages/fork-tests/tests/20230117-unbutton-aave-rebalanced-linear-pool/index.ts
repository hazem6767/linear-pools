...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const factory = await task.deployAndVerify('UnbuttonAaveLinearPoolFactory', args, from, force);
  ...
  if (task.mode === TaskMode.LIVE) {
    ...
    // UnbuttonAaveLinearPools require an UnbuttonAave Token
    const mockUnbuttonERC20Args = [input.WETH, 'DO NOT USE - Mock UnbuttonAave Token', 'TEST', 18];
    const mockUnbuttonERC20Token = await task.deployAndVerify('MockUnbuttonERC20', mockUnbuttonERC20Args, from, force);
    ...
    const mockPoolArgs = {
      ...
      wrappedToken: mockUnbuttonERC20Token.address,
    ...
    if (force || !task.output({ ensure: false })['MockUnbuttonAaveLinearPool']) {
      const PROTOCOL_ID = 0;
      ...
      await task.save({ MockUnbuttonAaveLinearPool: mockPoolAddress });
    }

    const mockPool = await task.instanceAt('UnbuttonAaveLinearPool', task.output()['MockUnbuttonAaveLinearPool']);
    ...
    await task.verify('UnbuttonAaveLinearPool', mockPool.address, [mockPoolArgs]);

    // We can also verify the Asset Manager
    await task.verify('UnbuttonAaveLinearPoolRebalancer', assetManagerAddress, [input.Vault, input.BalancerQueries]);
  }
};
