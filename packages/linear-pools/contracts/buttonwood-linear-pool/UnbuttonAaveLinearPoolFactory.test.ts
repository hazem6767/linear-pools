...
describe('UnbuttonAaveLinearPoolFactory', function () {
  ...
  beforeEach('deploy factory & tokens', async () => {
    ...
    const baseToken = await deployToken('AMPL', 18);

    // Deploy tokens
    const mainTokenInstance = await deployPackageContract('MockUnbuttonERC20', {
      args: [baseToken.address, 'wAMPL', 'wAMPL', 18],
    });

    const mainToken = await getPackageContractDeployedAt('TestToken', mainTokenInstance.address);

    const aaveWrappedTokenInstance = await deployPackageContract('MockAToken', {
      args: [baseToken.address, 'aaveAMPL', 'aaveAMPL', 18],
    })

    const aaveWrappedToken = await getPackageContractDeployedAt('TestToken', aaveWrappedTokenInstance.address);

    const wrappedTokenInstance = await deployPackageContract('MockUnbuttonERC20', {
    ...
    poolVersion = JSON.stringify({
      name: 'UnbuttonAaveLinearPool',
      version: '1',
      deployment: 'test-deployment',
    });
    factory = await deployPackageContract('UnbuttonAaveLinearPoolFactory', {
      args: [
        vault.address,
        ZERO_ADDRESS,
        queries.address,
  ...
  async function createPool(): Promise<Contract> {
    const wAMPL = await tokens.getTokenBySymbol('wAMPL');
    const wAaveAMPL = await tokens.getTokenBySymbol('wAaveAMPL');
    ...
    expectEvent.inReceipt(receipt, 'UnbuttonAaveLinearPoolCreated', {
      pool: event.args.pool,
      protocolId: UNBUTTONAAVE_PROTOCOL_ID,
    });

    return getPackageContractDeployedAt('UnbuttonAaveLinearPool', event.args.pool);
  }
  ...
  describe('constructor arguments', () => {
    ...
    it('registers tokens in the vault', async () => {
      ...
      expect(poolTokens.tokens).to.include(wAMPL.address);
      expect(poolTokens.tokens).to.include(wAaveAMPL.address);
    ...
    it('sets a rebalancer as the asset manager', async () => {
      ...
      const rebalancer = await getPackageContractDeployedAt('UnbuttonAaveLinearPoolRebalancer', assetManager);
    ...
    it('sets main token', async () => {
      const wAMPL = await tokens.getTokenBySymbol('wAMPL');
      expect(await pool.getMainToken()).to.equal(wAMPL.address);
    });

    it('sets wrapped token', async () => {
      const wAaveAMPL = await tokens.getTokenBySymbol('wAaveAMPL');
      expect(await pool.getWrappedToken()).to.equal(wAaveAMPL.address);
    });
  ...
  describe('protocol id', () => {
    it('should not allow adding protocols without permission', async () => {
      await expect(factory.registerProtocolId(UNBUTTONAAVE_PROTOCOL_ID, 'Buttonwood')).to.be.revertedWith('BAL#401');
    });

    context('with no registered protocols', () => {
      it('should revert when asking for an unregistered protocol name', async () => {
        await expect(factory.getProtocolName(UNBUTTONAAVE_PROTOCOL_ID)).to.be.revertedWith('Protocol ID not registered');
      });
    });
    ...
    context('with registered protocols', () => {
      ...
      beforeEach('register some protocols', async () => {
        ...
        await factory.connect(admin).registerProtocolId(UNBUTTONAAVE_PROTOCOL_ID, UNBUTTONAAVE_PROTOCOL_NAME);
      });
      ...
      it('protocol ID registration should emit an event', async () => {
        ...
        expectEvent.inReceipt(await tx.wait(), 'UnbuttonAaveLinearPoolProtocolIdRegistered', {
      ...
      it('should register protocols', async () => {
        ...
        expect(await factory.getProtocolName(UNBUTTONAAVE_PROTOCOL_ID)).to.equal(UNBUTTONAAVE_PROTOCOL_NAME);
      });

      it('should fail when a protocol is already registered', async () => {
        await expect(
          factory.connect(admin).registerProtocolId(BEEFY_PROTOCOL_ID, 'Random protocol')
